...
public class XSDReader {
	...
	// 解析XSD，返回数据节点对象列表
	public List<XSDNode> paserXSD(String xsd, int i) throws Exception {
		SAXReader saxReader = new SAXReader();
		// ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(
		// xsd.getBytes(XMLConstants.ENCODING));
		Document doc = saxReader.read(xsd);
		Element element = doc.getRootElement();
		String basePath = null;
		Element dataElement = null;
		if ("".equals(XMLConstants.XSD_DEFAULT_NAMESPACE)) {
			if ("".equals(XMLConstants.MESSAGE[i])) {
				dataElement = element;
			} else {
				basePath = "//element[@name=\"" + XMLConstants.MESSAGE[i]
						+ "\"]";
				dataElement = (Element) element.selectSingleNode(basePath);
			}
		} else {
			basePath = "//" + XMLConstants.XSD_DEFAULT_NAMESPACE
					+ ":element[@name=\"" + XMLConstants.MESSAGE[i] + "\"]";
			dataElement = (Element) element.selectSingleNode(basePath);
		}
		...
		if ("".equals(XMLConstants.XSD_DEFAULT_NAMESPACE)) {
			...
			elementPath = "//" + XMLConstants.XSD_DEFAULT_NAMESPACE
					+ ":element";
		}
	...
	public void paseData(Element element, String xPath, String xsdPath,
		...
		if (maxOccurs != null && !"1".equals(maxOccurs)
				&& !("//" + XMLConstants.MESSAGE + "").equals(xPath)) {// 节点可以有多个
		...
		if (elementNodes != null && elementNodes.size() > 0) {// 如果下面还有element,说明不是叶子
			...
			Node annotationNode = element.selectSingleNode(xsdPath
					+ "[@name=\"" + nodeName + "\"]/"
					+ XMLConstants.XSD_DEFAULT_NAMESPACE + ":annotation/"
					+ XMLConstants.XSD_DEFAULT_NAMESPACE + ":documentation");
